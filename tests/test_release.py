...
class ReleaseTests(unittest.TestCase):
    def inputs(self, root):
        ...
        for name in ("robot_reel-0.6.0-py3-none-any.whl", "robot_reel-0.6.0.tar.gz"):
    ...
    def test_tested_bytes_and_all_asset_checksums_are_preserved(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertIn("Export reviews.", notes)
        ...
        for mutation in ("missing", "old version", "extra", "symlink", "nonempty output"):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as folder:
                ...
                if mutation == "missing":
                    ...
                    (dist/"robot_reel-0.6.0.tar.gz").rename(dist/"robot_reel-0.5.0.tar.gz")
                elif mutation == "extra":
