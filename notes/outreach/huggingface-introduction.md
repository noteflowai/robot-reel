...
[Open paired outcomes](https://glayguo-robot-reel.static.hf.space/stress/index.html#outcomes). The current source CLI independently verifies the exported JSON against the full collection. These are descriptive results from one task, not a significance test or general robustness claim. The 0.7.1 release ZIP remains the older viewer; see the [current-source instructions](https://github.com/noteflowai/robot-reel/blob/main/docs/stress.md#compare-paired-outcomes).
