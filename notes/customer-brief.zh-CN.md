...
0.6.0 将完整离线实验包、示例复盘记录和快速上手说明随版本一起交付：
下载解压后即可用浏览器查看真实录制，无需 GPU 或 Python；安装对应版本后，
还能独立核验复盘事实。发行包由实际安装的程序导出，再以同一份文件完成桌面、
移动端和断网浏览器检查，方便团队将演示带回自己的环境体验、交流和二次开发。
容器运行方式继续支持普通用户和挂载目录写入。
...
0.7.0 安装包已包含布料数据校验和完整离线导出，客户无需源码、Newton
或 GPU 即可复用现有录制；重新仿真仍需可选 Newton 环境。既有 0.6.0
发布文件保持原内容。
