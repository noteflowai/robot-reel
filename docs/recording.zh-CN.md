...
以下命令从仓库根目录执行。视觉概览与全部演示见[项目首页](../README.zh-CN.md)。VLA、MCP 导演和 Newton 的独立流程分别见 [VLA 指南](vla.md)、[导演指南](director.md)和 [Newton 指南](newton.md)。
...
的关系，以及哪些还没有集成，见[汽车集成说明](automotive.md)。
...
CI 还会检查真实 MCP 协议连接。
安装包会在源码目录外实际导出完整实验，容器会检查普通用户与挂载目录写入；
这六组验证全部通过后才部署首页或发布版本，见[安装与发布说明](distribution.md)。
